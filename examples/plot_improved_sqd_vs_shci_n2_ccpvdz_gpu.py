...
CHEM = 1.6e-3
NPY = os.path.join(BASE, "_plot_data_sqd_vs_shci_n2_ccpvdz_gpu.npy")
NCAS, NELEC = 10, 10
    ...
    if not P["shci"]:
        ...
        for eps in [1e-1, 9e-2, 8.5e-2, 8e-2, 7.5e-2, 7.3e-2, 7.2e-2, 7.1e-2,
            ...
            e_t, e_pt2, dim = tc_sqd.solve_hci(
                ...
                max_iter=15, ecore=ecore, return_details=True, verbose=False, backend="gpu")
...
def _plot(P):
    ...
    ax.set_title("improved SQD (GPU hybrid) vs SHCI (N2/cc-pVDZ, R=3.0 A)", fontsize=12)
    ...
    out = os.path.join(BASE, "fig_improved_sqd_vs_shci_n2_ccpvdz_gpu.png")
    fig.tight_layout()
