...
## Round 013：eigsh_tol 收敛容差 + active 批量消融（2026-08-21，已验证）

### 消融实测（固定输入）

**单杠杆**（bench_round013_tol_batch，tol/n_active 分别扫）：

| 杠杆 | 配置 | n_mv | wall比* | err |
|---|---|---|---|---|
| GPU 12,12 closure tol | 1e-10 → **1e-6** | 2773 → **1253** (2.2×) | **0.54×** | 1.43e-10 (不降) |
| GPU 12,12 closure n_active | 30 → **90** | 2713 → **1459** (1.9×) | **0.62×** | 2.25e-10 (不变) |
| CPU 10o closure tol | 0.0 → 1e-10 | 827 → 516 | 0.59× | 1.58e-10 (不变) |
| CPU 10o closure tol | 0.0 → **1e-8** | 827 → 426 | 0.47× | 1.59e-10 (不变) |

**联合配方实测**（bench_round013_fix，M2 修复：替代原乘积外推）：

| 配置 | n_mv | wall | err |
|---|---|---|---|
| GPU 12,12 closure baseline | 2693 | 121.2s | 2.25e-10 |
| **recipe(1e-6 + n_active=90)** | **699** | **44.7s** | **1.13e-10** |

联合实测 n_mv 0.26×（3.9× 少）、wall 0.37×（2.7× 快），err 1.13e-10（不降）。
原"单杠杆乘积外推 0.54×0.62=0.33× / 2.2×1.9=4.2×"略乐观（杠杆部分重叠 + 非
diag 固定开销），实测 0.37× / 3.9× 更准。

\*wall 比跨进程不可比（本机负载波动 131↔377s）；n_mv 是确定性主指标。CPU 行
用 bench_round013_fix 显式 `eigsh_tol=0.0`（可复现；原 bench_round013_tol_batch
用 `None` 在 54b0f65 后语义漂移为 1e-10，已弃用）。

### 产品化（commit 54b0f65）

- **CPU/except 分支 eigsh 默认 tol 0 → 1e-10**：round_005（1.46×，E diff ~1e-13）+
  本轮（10o closure n_mv 0.62×）双实验验证，P0 门槛满足；E diff ≤1e-13 全库
  isclose 锚吸收（注：能量跨版本漂移 1e-13 成立；同配置独立 ARPACK 运行间本征矢
  diff ~1e-9，已被 fallback 测试 atol 放宽到 1e-8 吸收，见 fcbc8ba）。
- **闭包加速配方**（文档推荐，不改默认）：`coverage_closure=True` + `eigsh_tol=1e-6` +
  `n_active_per_round=90` → GPU 联合实测 wall 0.37× (2.7×)，err 1.13e-10（不降）。
- `eigsh_tol` 参数暴露到 _Subspace/active/ev/best（默认 None = 逐分支 1e-10）。

### 认知更新

- **ARPACK 相对容差的停机界远悲观于实际收敛**：tol=1e-6 的界是 |E|·1e-6 ≈ 1e-4，
  实际 err 1.43e-10 —— 放松 tol 到 1e-6 对闭包路径零精度损失。
- **active 批量是独立杠杆**：n_active=90 减少中间轮次/次数，闭包补全后最终结果不变。
- **非闭包路径未验证**：tol=1e-6/n_active=90 的推荐限于 closure 配方；非闭包
  （残余误差由缺串主导）用户慎用。
...
## Round 015：低 shots closure 稳健性（2026-08-28，已验证——shots 轴坍缩）

### 实测（bench_round015_lshots，配方 closure+1e-6+n_active=90，GPU warm）

| shots | baseline dim | baseline err | **closure dim** | **closure err** | closure wall |
|---|---|---|---|---|---|
| 500 | 824,464 | 3.63e-7 | **853,776** | **1.13e-10** | 51.4s |
| 100 | 348,100 | 2.76e-4 | **853,776** | **1.83e-10** | 55.1s |
| 50 | 341,056 | 2.60e-4 | **853,776** | **1.83e-10** | 51.5s |
| 20 | 348,100 | 2.76e-4 | **853,776** | **1.81e-10** | 46.7s |

10o 旁证（CPU）：@100/@50/@20 全部补全到 63,504 全空间，err 1.58e-10。
P0/P1/P1' 全 PASS。

### 核心发现：closure 使结果 shots 无关

- **@20 与 @500 给出同样的精确 FCI**（err 1.81e-10 vs 1.13e-10，全 853,776）；
  不闭包时 @20 err 2.76e-4 → 闭包后 **1525× 改善**。
- wall 平坦（47-55s）：closure 的全空间 diag 主导，active 循环低 shots 只是
  起点更小（str_seq: 低 shots 从 154 爬到 908——**确定性 PT2 选态与 shots
  无关地爬到 908**，closure 补最后 16）。
- **采样角色坍缩为种子**：量子 shots 对 12,12 最终精度已无贡献（20 shots ≈ 500
  shots），确定性 PT2 循环 + BFS 闭包完成全部工作。这是 round_012 认知
  （"闭包=采样+确定性混合"）的最强确认。

### 适用边界（配方声明）

- 配方要求**全字符串空间可对角化**（12,12 = 924 串 = 853k dim，GPU 可行）；
  更大活性空间（如 (16,16) = 12,870 串 = 166M dim）不可行时须给 `max_strings`
  截断，shots 轴坍缩结论不外推。
- 低 shots 下 active 循环的爬升依赖 PT2 选态确定性（10 轮 × n_active/轮）；
  极低 shots（<20）未测。
- 采样式 SQD 的 shots 价值主张在此体系上不再成立（对 shots ≤500）；
  若研究目标是"少 shots 量子优势"，应聚焦全空间不可对角化的大体系。

> 勘误（2026-08-28 M0，追加不改写）：本节上表 @20 行的改善倍数应为 **~1.5e6×**
> （2.76e-4 / 1.81e-10），此前写 1525× 系 e3 滑位（对应误用 1.81e-7）；
> summary.md / STATE.md 已同步更正。

---

## Round 016：北极星正式验收——三指标全部达成（2026-08-28，已验证）

### 北极星三指标（12,12，验收配置=@500 + 闭包配方）

| 指标 | 要求 | 实测 | 判定 |
|---|---|---|---|
| 精度 | err ≤3×SHCI（1.14e-10，锚 3.8e-11@829,921） | 1.13e-10 = 2.97× | **压线达成** |
| 速度 | wall ≤3×SHCI（2436s） | 41.9-43.5s = 0.05× | 大余量 |
| 采样 | shots ≤500 | 500（round_015 证 @20 亦可） | 达成 |
| 不回归 | 其余体系 | 4 体系全空间，err ≤ baseline+1e-12 | PASS |

不回归矩阵：n2_ccpvdz_10o 1.59e-10、c2_sto3g 2.42e-13、n2_sto3g 4.26e-14、
c2_ccpvdz_10o 1.42e-13——全部达全空间，对各自历史 SHCI 参考比值
1.67 / 380× 优 / 2e4× 优 / 0.55。

### P2 底噪微实验（证伪）

tol=1e-6：1.13e-10（n_mv 699）< tol=1e-10：2.25e-10（n_mv 1459）≈ tol=0：
2.25e-10（n_mv 2639），三轮复现。**底噪 ∝ 累计 matvec 舍入积累，与收敛容差
无关**——更少 matvec 反而更准。锁定 @500 + tol=1e-6 为验收配置；2.97× 零余量。

### 口径判定

全空间=FCI 达成 → 同任务比较（853,776 vs SHCI 锚 829,921 维度不严格相等；
全空间能量对同库参考是绝对判据）。残差 = eigsh 数值底噪，非覆盖误差；
"闭包=采样+确定性混合方案，非纯采样突破"定性（round_012）维持。

### 参考生成器坑（数据纪律更新）

`pyscf direct_spin1.kernel(conv_tol=1e-12)` 作强关联体系真基态参考不可靠：
C2/cc-pVDZ CAS(10,10) **根跳**（-75.5509，比真基态高 9.3e-3，conv_tol=1e-13
才不跳）；N2/STO-3G (7,7) **跨运行非确定漂移 ~1e-9**（三次三个值）。
修正：验收级参考统一库内 `solve_sci` 全空间（与严格 direct_spin1 一致 ~1e-13）。

### 结论

**北极星达成**（12,12 err ≤3×SHCI + wall ≤3× + shots ≤500 + 其余不回归）。
...
## Round 021：闭包边界探索——(14,14)=11.78M 维截断 closure 行为刻画（2026-09-01，已验证）

### 体系与方法

N₂/cc-pVDZ R=3.0 Å 全电子 (14e,14o)（n_core=0, n_virtual=14；全空间
C(14,7)²=11,778,624 维——全空间不可入实验循环的最大可考据档）。参考 = 库内
solve_sci 全空间真基态（E_total=-108.7818876902919，1491 mv，5603s，一次性
CPU 后台；pyscf selected_ci 流式 C 核内存仅 ~1GB——R1 源码级确认无 t1 巨阵）。
src/ 零改动（纯行为刻画轮，同 round_015 形态）。

### 预测对照（theory §3 + 附录 A 定版；数据 benchmarks/_round021_closure.json）

| # | 判定 | 实测 |
|---|---|---|
| P0（核心：ms=2500 同预算 err 比 ≥2.0 / <1.3 证伪） | **证实（机制修正）** | err_on=9.11e-8 vs err_off=4.92e-5 = **539×**；副档 2000 = 20.9×。**但机制非 R2 设想的"同 dim 选态质量"**：OFF 臂在 1517 串内禀停滞（58.3 万候选全部 \|pt2\|<1e-7 地板；ms=2000/2500 轨迹逐位全同=预算从不约束），closure 的价值 = **破停滞**——OFF 永卡 4.9e-5 平台，ON 随预算继续收敛 |
| P0a（前提） | PASS | 全空间 matvec 1.79s（R1 外推高估 ~50×）；ms=200 短跑无异常；BFS 成本微基准可控 |
| P0'（零回归） | **证实** | CPU 235 passed + 1 failed（test_spin_resolved basis_invariance——**新 flake**，非 round_020 六例家族；隔离复跑 3/3 通过）+ GPU 10p/1xf/1xp 全绿；src/tests 零改动，隔离即过排除回归 |
| P1（收敛形态） | **部分证实** | ON err 9.97e-5→2.35e-6→9.11e-8 与 sigma² 单调 ✓；sigma²_off/on@2000=16.8 ✓；但 **ms=1500 同 dim 反转**：OFF 优 1.67×（err）、sigma² 倒挂 2.63×——机制转型点（BFS 高阶配额 238/1500=16% 太薄时字符串级聚合选态反而略逊于 det 级贪心） |
| P1b（shots 轴复活） | **证实（弱效应）** | err(@100)/err(@500)：OFF 1.29>1.2 ✓（ON 1.12 重试带）；closure 收益比 @100≥@500 ✓。对比 12,12 的全坍缩：不可对角化体系 shots 轴**部分复活**（10-30% 效应） |
| P1c（语义对照 ms=100） | **证实（注记）** | 逐轮轨迹 dim 六点全同、双零注入、终 dim 同；ΔE=2.33e-11 = tol=1e-6 停机噪声随维度放大（smoke 底 5.5e-12@3.3e4 → 此处 1.4e6）；n_mv 差恰=ON 播种 diag 的 21 次（结构归因闭环） |
| P2（机制诊断） | **证实** | 1-hop 池覆盖率恰 1.000（2,738,540=2170×1262）；生产性注入三档全恰 1 层；≥3 阶占比 76/86/91% |

### 关键新认知

1. **截断 closure 的激活先决条件**（R2 §2.0 预言，P1c+P0a 双场景证实）：BFS 与
   PT2 共用 max_strings 门控，PT2 不先在预算前停下（轮数帽/真性停滞）则闭包
   零激活。12,12 靠真性停滞（908/924），(14,14) 靠轮数帽（max_rounds=5）。
2. **OFF（纯 PT2 贪心）在 11.78M 维真性停滞于 1517 串**（补集 2170 串、
   58.3 万候选全低于 pt2_floor=1e-7）：S/D-from-dominant 选态在大空间够不到
   高激发阶——round_008 缺失串机制的**大体系放大版**。closure 的 BFS 以
   字符串级聚合打分单层补 1238 串直达预算，把 err 从平台 4.9e-5 压到 9.1e-8。
3. **同 dim 选态质量反转点**：ms=1500 OFF 反优 1.67×——BFS 高阶填充在薄配额下
   未兑现收益；配额 ≥37%（2000 档）起反转为 21×/539×。
4. **evpt2 在截断下仍改善 2.3-11.6×**（12 run 全）：终值 e_pt2 来自 cand_all
   全枚举（cipsi.py:1609-1618 不受预算门控），R2 担忧的失真未实现。
5. **配方边界（P0a 发现）**：tail_suppression 在大体系不可用——尾部发现每轮
   抽新位串 + 采样串不受预算门控 → 串数洪流（实测 GPU t1 15.7GB 逼近 16GB
   上限）。12,12 串池 924 快速饱和故无害；(14,14) 池 3432 不饱和即泛滥。
   **大体系配方须 tail_suppression=False**。
6. **cupy 内存池跨 run 胀破**：t1 随串数逐档累积 → 16GB 显存 OOM。护栏 =
   diag 后 free_all_blocks + 每点独立进程（bench 侧，已修）。
7. **HCI 参照在 (14,14) 不可得**：eps=1e-2 单点超 30min 帽，按 R10 舍弃。

### R4 过程记录

静态审查抓 1 blocker（BFS 快照差集 off-by-one → 判别档将误判零激活；smoke 零
注入形态恰好掩盖——测试形态覆盖教训）+ 3 major（diag 时序前提错 / ref tol
字段不实 / P1c 阈值随噪声底修订），R3 修正循环后复验放行（R4 用 ast 抽取
修复函数独立三形态验证）。第 2 段全库测试结果见 P0' 行。
