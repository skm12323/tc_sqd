...
换基→重解，`solve_sqd_natural_orbitals`）突破配置恢复的 MO 基覆盖瓶颈，能量误差最多改善
263×（纯经典后处理）。CCSD-NO 弱关联可行、强关联打折（自身多参考缺失），指向自洽迭代为
最终形态。详见 REVIEW 方向①。

**主动采样已落地**（2026-08-04，`solve_sqd_active` + 验证）：采样↔选态双闭环，受限 PT2
选态（AS-SQD）确定性补足采样缺口。N₂/STO-3G 拉伸低采样 n=100：纯采样误差 ~2e-3（超化学
精度）→ 主动采样 **3.95e-7**（无需额外量子测量）；max_strings 约束 PT2 扩展使子空间远小于
全空间仍达化学精度（区别于 solve_cipsi 补全全空间）。C₂ 覆盖率不稳（3/8 失败）的根治：PT2
选态确定性保证关键 det 必进子空间。详见 REVIEW 方向②。

**拟 Krylov 理论化已落地**（2026-08-04，文档）：把方向 A 多 scale UCJ 形式化为参数化
Krylov 型子空间 V=span{Û(s)e^{iĴ(s)}Û(s)†|HF⟩}——与 SKQD 的 e^{−ikHΔt} 同构（Ĵ 为
H 二体部分，硬件可编译的"近似 H 演化"）。收敛论证：E_V 误差 ≤ ‖(I−P_V)|Ψ_gs⟩‖²·ΔE，
...
- **实测**：σ-vector == 稠密 `build_ci_matrix`（H₂/LiH/N₂ ≤1e-8）；cupy matvec
  ...
  ⚠ T 表 einsum 版**大维度标度差于 pyscf C 核**（O(M·na²·nb) 立方 vs C 核 O(nnz)）：
  dim 1.6×10⁵ 落后 50×。
- **linkstr RawKernel 版**（`sigma_linkstr_gpu`，2026-08-11）**真正超越 pyscf C 核**
  direct_spin1.contract_2e：全空间 matvec **3-13× 快**（N₂/STO-3G 全 0.88ms vs 2.68ms；
  12o 子空间 1.11ms vs 15.08ms = 13.6×）。⚠ 仅全空间正确（linkstr 中间态需全空间），
  子空间（SQD）下与 selected_ci 差 ~2 Ha，故 `solve_sci(backend="gpu")` 仍用 T 表版
  （子空间正确），`sigma_linkstr_gpu` 为独立全空间快速 API。子空间修正需全空间 mid
  （t1=norb²·na_full·nb 内存爆炸），未做。
